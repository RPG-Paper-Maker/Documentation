...
* **Реакції при відкритті карти**: Тут можна налаштувати конкретні речі, які будуть виконуватися при заході на крату. Пізніше ми покаемо, як це можна налаштувати через об'єкти.
* **Битви:**

![](../.gitbook/assets/map-properties-battles.png)

* `№ бойової карти`: Номер бойовой карти, який буде використовуватися при початку битви на цій карті \(якщо вибрані стандартні налаштування\)
* **Випадкові битви**:
  * `Список загонів`: 
...
* `Приорітет`: Пріорітет цього загону
* `Імовірність`: Розрахована імовірність зустріти саме цей загон на основі пріоритетів всіх загонів
...
* Моментально перенести курсор на квадрат вибранного елементу можна натисканням `CTRL + Ліва кнопка миші`.
...
Область вибора елемента карти **\(3\)** дозволяє вибрати, який елемент розміщатиметься на карті \(підлоги, спрайти, об'єкти, рельєфи тощо\). Ви також можете додати або видалити щось на певному квадраті, натиснувши лівою або правою кнопкою миші.
...
* `Все`: Записує зміни на всіх редагованих картах. Якщо карта була відредагована, після її назви буде показуватисся зірочка `*`:
...
В області вибора текстур **\(2\)** треба вибрати стіну:
...
В області вибору текстури **\(2\)** є декілька налаштуань для додавання гір:

![](../.gitbook/assets/mountains-texture.png)

* `Підлога зверху`: Підлога, яка буде автоматично додана на верхню частину гір. Натисність на кнопку `...`, щоб відредагувати підлогу зверху.
* `Ширина межі`: Якщо ширина межі довінює нулю, гора буде під кутом 90°. Це налаштування дозволяє вказати ширину меж гір \(щоб створити менші схили\).
...
В 3D трансформаціє є наступними трьома різними діями:
...
Якщо ви натисните по елементу карти лівою або правою кнопкою миші, цей об'єкт буде виділено, і до нього буде застосовано вибрані налаштування трансформації. Ви можете вказати поворот по осях `X`, `Y` або `Z`, який буде застосовано при натисканні.
...
* `Застосувати правою/лівою кнопкою миші`: Якщо ця опція вибрана, трансформацію буде застосовано при натисненні на об'єкт. Якщо вибрана операція `+`, права кнопка миші буде робити зворотню операцію `-`. Якщо ця опція не вибрана, вибраний об'єкт мінятиметься зразу ж при зміні поля `Кут`.
